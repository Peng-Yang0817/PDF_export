...
namespace JctvSC.Areas.NForm.Models.Services
{
    public class itextsharpService_fun
    {
        public MemoryStream testPdf()
        {
            ...
            document.Open();
            //標題
            document.Add(setString(" 技專校院【五專入學招生名額分配】 ", 12, Element.ALIGN_CENTER, Font.BOLD));

            //學校_PS
            PdfPTable pTableSchool_PS = new PdfPTable(new float[] { 1f });
            pTableSchool_PS.AddCell(setCell("    學校:    臺北科技大學    學校代碼:    104    學年度:   111  \n", 1, 1, Element.ALIGN_LEFT, Font.NORMAL, 12, 15f));
